...
Descripción:
- Realiza validación cruzada con splits de entrenamiento generados aleatoriamente, pero con sets de validación fijos.
- Cada iteración selecciona aleatoriamente N_TRAIN datasets para ajuste, dejando los VALIDATION_IDS fijos para validación.
- Guarda resultados detallados de cada iteración, incluyendo costos de ajuste y validación, parámetros encontrados, y razones de terminación.
- Utiliza ProcessPoolExecutor para paralelizar las iteraciones.
Configuración:
- N_ITERATIONS: Número de iteraciones (splits) a ejecutar.
- VALIDATION_IDS: IDs de datasets que se usarán como sets de validación fijos.
- N_TRAIN: Número de datasets a usar para ajuste en cada iteración.
- MAX_WORKERS: Número máximo de procesos paralelos.
- OUTPUT_BASENAME: Base para los nombres de archivos de salida.
- MODEL_STRUCTURE: Estructura del modelo a usar en la optimización.
- CUSTOM_PSO_CONFIG: Configuración personalizada para el algoritmo PSO de pymoo.
Datasets:
- Se define una lista de datasets con su ID, nombre y ruta al archivo Excel.
"""
