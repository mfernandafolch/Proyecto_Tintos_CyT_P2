"""
Módulo de optimización con PSO (pymoo) para ajustar parámetros del modelo
de fermentación Zenteno modificado con azúcar total.

Modelo dinámico modificado:
    Estados: x = [X, N, S, E]

    X : biomasa viable (g/L)
    N : nitrógeno asimilable (g/L)
    S : azúcar total fermentable (g/L)
    E : etanol (g/L)

Parámetros del modelo:
    p = [mu0, betaS0, Kn0, Ks0, Kie0, Kd0, Yxn, Yxs, Yes]

Parámetros libres:
    theta = [mu0, Kn0, Kie0, Kd0, Yxn]

Parámetros fijos:
    betaS0, Ks0, Yxs, Yes

Este archivo define:
- El orden y los límites de los parámetros del modelo.
- La estructura de parámetros libres y fijos.
- La simulación del sistema a partir de un vector theta.
- La función objetivo para uno o varios datasets.
- El problema de optimización para pymoo.
- Una condición de parada por brecha relativa.
- La función principal run_pymoo_estimation.
- Una utilidad para graficar convergencia.
"""

import os
import sys
import numpy as np
import matplotlib.pyplot as plt

from pymoo.core.problem import Problem
from pymoo.algorithms.soo.nonconvex.pso import PSO as PymooPSO
from pymoo.optimize import minimize
from pymoo.core.termination import Termination
from pymoo.termination import get_termination
from pymoo.termination.collection import TerminationCollection


# =============================================================================
# 1. Imports del proyecto
# =============================================================================

# CURRENT_DIR = os.path.dirname(__file__)
# PROJECT_DIR = os.path.abspath(os.path.join(CURRENT_DIR, "..", ".."))

# if PROJECT_DIR not in sys.path:
#     sys.path.insert(0, PROJECT_DIR)

from simulacion_v2 import simulate_system


# =============================================================================
# 2. Parámetros del modelo modificado
# =============================================================================

PARAM_ORDER = [
    "mu0",
    "betaS0",
    "Kn0",
    "Ks0",
    "Kie0",
    "Kd0",
    "Yxn",
    "Yxs",
    "Yes",
]


BOUNDS_DICT = {
    "mu0": (1e-2, 1.0),
    "betaS0": (1e-2, 10.0),
    "Kn0": (1e-3, 1.0),
    "Ks0": (1e-1, 100.0),
    "Kie0": (1e-1, 100.0),
    "Kd0": (1e-8, 1e-2),
    "Yxn": (1e-1, 10.0),
    "Yxs": (1e-1, 10.0),
    "Yes": (1e-1, 10.0),
}


# =============================================================================
# 3. Configuración PSO
# =============================================================================

PSO_CONFIG = {
    "epoch": 1000,
    "pop_size": 25,
    "w": 0.7,
    "c1": 1.5,
    "c2": 1.5,
    "seed": 123,
    "verbose": True,
    "save_history": True,
    "relative_gap_threshold": 0.01,
}


# =============================================================================
# 4. Estructura del modelo
# =============================================================================
# Los parámetros libres se entregan solo como nombres, porque PSO no usa valores iniciales.
# Los parámetros fijos mantienen valores conocidos a partir del modelo original.

MODEL_2264 = {
    "fixed": {
        "mu0": 0.277041, 
        "Kn0": 0.009647,
        "Kie0": 42.528284,
        "Kd0": 0.0001,
        # "Yxn": 28.923223,
    },
    "free": ["betaS0", "Ks0", "Yxn","Yxs", "Yes"],
}

# =============================================================================
# 5. Utilidades de parámetros
# =============================================================================

def prepare_model_structure(model_structure):
    """
    Separa parámetros fijos y libres.

    Parameters
    ----------
    model_structure : dict
        Diccionario con llaves:
        - "fixed": dict con parámetros fijos y sus valores.
        - "free": lista con nombres de parámetros libres.

    Returns
    -------
    fixed_params : dict
        Parámetros fijos con sus valores.

    free_names : list
        Nombres de parámetros libres.
    """

    fixed_params = model_structure["fixed"].copy()
    free_names = list(model_structure["free"])

    repeated = set(fixed_params.keys()).intersection(set(free_names))
    if repeated:
        raise ValueError(
            f"Hay parámetros repetidos entre fixed y free: {sorted(repeated)}"
        )

    unknown_fixed = [name for name in fixed_params if name not in PARAM_ORDER]
    if unknown_fixed:
        raise ValueError(
            f"Parámetros fijos no reconocidos en PARAM_ORDER: {unknown_fixed}"
        )

    unknown_free = [name for name in free_names if name not in PARAM_ORDER]
    if unknown_free:
        raise ValueError(
            f"Parámetros libres no reconocidos en PARAM_ORDER: {unknown_free}"
        )

    missing = [
        name for name in PARAM_ORDER
        if name not in fixed_params and name not in free_names
    ]

    if missing:
        raise ValueError(
            f"Faltan parámetros para completar el modelo: {missing}"
        )

    return fixed_params, free_names


def build_full_params(theta, free_names, fixed_params):
    """
    Construye el diccionario completo de parámetros a partir de theta.
    """

    theta = np.asarray(theta, dtype=float).reshape(-1)

    if len(theta) != len(free_names):
        raise ValueError(
            f"theta tiene largo {len(theta)}, pero se esperaban "
            f"{len(free_names)} parámetros libres."
        )

    params = fixed_params.copy()

    for name, value in zip(free_names, theta):
        params[name] = float(value)

    return params


def params_dict_to_vector(params_dict, param_order=PARAM_ORDER):
    """
    Convierte un diccionario de parámetros al vector que recibe el modelo dinámico.
    """

    missing = [name for name in param_order if name not in params_dict]

    if missing:
        raise ValueError(
            f"Faltan parámetros para construir el vector completo: {missing}"
        )

    return np.array([params_dict[name] for name in param_order], dtype=float)


def build_bounds_for_free_params(free_names, bounds_dict=BOUNDS_DICT):
    """
    Construye los límites inferiores y superiores para los parámetros libres.
    """

    bounds = []

    for name in free_names:
        if name not in bounds_dict:
            raise ValueError(f"No hay bounds definidos para '{name}'")

        bounds.append(bounds_dict[name])

    return bounds


def get_pso_config(custom_config=None):
    """
    Retorna configuración de PSO, permitiendo sobrescribir valores.
    """

    config = PSO_CONFIG.copy()

    if custom_config is not None:
        config.update(custom_config)

    return config


# =============================================================================
# 6. Simulación desde theta
# =============================================================================

def simulate_from_theta(
    theta,
    free_names,
    fixed_params,
    x0,
    t_rel,
    temp,
    Nadd,
    t_span
):
    """
    Simula el modelo modificado a partir de un vector theta.

    El modelo modificado retorna estados:
        y[:, 0] = X
        y[:, 1] = N
        y[:, 2] = S
        y[:, 3] = E

    Returns
    -------
    sim : OdeResult
        Resultado de solve_ivp.

    S_sim : np.ndarray
        Perfil simulado de azúcar total.

    Et_final_sim : float
        Etanol final simulado.
    """

    params_dict = build_full_params(
        theta=theta,
        free_names=free_names,
        fixed_params=fixed_params
    )

    params_vector = params_dict_to_vector(
        params_dict=params_dict,
        param_order=PARAM_ORDER
    )

    sim = simulate_system(
        x0=x0,
        t_rel=t_rel,
        temp=temp,
        Nadd=Nadd,
        tspan=t_span,
        params_list=params_vector
    )

    if not sim.success:
        raise RuntimeError(f"Simulación fallida: {sim.message}")

    y = sim.y.T

    if y.shape[1] != 4:
        raise ValueError(
            f"El modelo modificado debe retornar 4 estados [X, N, S, E], "
            f"pero retornó {y.shape[1]}."
        )

    S_sim = np.asarray(y[:, 2], dtype=float)
    E_sim = np.asarray(y[:, 3], dtype=float)

    Et_final_sim = float(E_sim[-1])

    return sim, S_sim, Et_final_sim


# =============================================================================
# 7. Función objetivo
# =============================================================================

def compute_objective_breakdown(
    theta,
    free_names,
    fixed_params,
    x0,
    t_rel,
    temp,
    Nadd,
    t_span,
    sugars_profile,
    Et_final_exp,
    penalty=1e12,
    eps=1e-8
):
    """
    Calcula el desglose de la función objetivo para una fermentación.

    La función objetivo usa:
    - Error normalizado medio del perfil de azúcar total S.
    - Error normalizado del etanol final E.

    objective_total = mean(error_S^2) + error_E_final^2
    """

    try:
        _, S_sim, Et_final_sim = simulate_from_theta(
            theta=theta,
            free_names=free_names,
            fixed_params=fixed_params,
            x0=x0,
            t_rel=t_rel,
            temp=temp,
            Nadd=Nadd,
            t_span=t_span
        )

        sugars_profile = np.asarray(sugars_profile, dtype=float)

        if len(S_sim) != len(sugars_profile):
            raise ValueError(
                f"Largo incompatible entre S simulado ({len(S_sim)}) "
                f"y perfil experimental ({len(sugars_profile)})."
            )

        if not (np.all(np.isfinite(S_sim)) and np.isfinite(Et_final_sim)):
            return {
                "sugar_residual_vector": np.full(len(sugars_profile), np.nan),
                "ethanol_residual": np.nan,
                "sugar_error_sum": penalty,
                "sugar_error_mean": penalty,
                "ethanol_error": penalty,
                "objective_total": penalty,
                "sugars_sim": S_sim,
                "Et_final_sim": Et_final_sim,
                "sugar_scale": np.nan,
                "ethanol_scale": np.nan,
            }

        sugar_scale = max(np.nanmax(np.abs(sugars_profile)), eps)
        ethanol_scale = max(abs(float(Et_final_exp)), eps)

        sugar_res = (S_sim - sugars_profile) / sugar_scale
        etoh_res = (Et_final_sim - float(Et_final_exp)) / ethanol_scale

        sugar_error_sum = np.sum(sugar_res ** 2)
        sugar_error_mean = sugar_error_sum / len(S_sim)
        ethanol_error = etoh_res ** 2

        objective_total = sugar_error_mean + ethanol_error

        return {
            "sugar_residual_vector": sugar_res,
            "ethanol_residual": float(etoh_res),
            "sugar_error_sum": float(sugar_error_sum),
            "sugar_error_mean": float(sugar_error_mean),
            "ethanol_error": float(ethanol_error),
            "objective_total": float(objective_total),
            "sugars_sim": S_sim,
            "Et_final_sim": float(Et_final_sim),
            "sugar_scale": float(sugar_scale),
            "ethanol_scale": float(ethanol_scale),
        }

    except Exception:
        return {
            "sugar_residual_vector": np.full(len(sugars_profile), np.nan),
            "ethanol_residual": np.nan,
            "sugar_error_sum": penalty,
            "sugar_error_mean": penalty,
            "ethanol_error": penalty,
            "objective_total": penalty,
            "sugars_sim": None,
            "Et_final_sim": np.nan,
            "sugar_scale": np.nan,
            "ethanol_scale": np.nan,
        }


def objective_function(
    theta,
    free_names,
    fixed_params,
    x0,
    t_rel,
    temp,
    Nadd,
    t_span,
    sugars_profile,
    Et_final_exp,
    penalty=1e12
):
    """
    Retorna solo el valor total de la función objetivo para una fermentación.
    """

    breakdown = compute_objective_breakdown(
        theta=theta,
        free_names=free_names,
        fixed_params=fixed_params,
        x0=x0,
        t_rel=t_rel,
        temp=temp,
        Nadd=Nadd,
        t_span=t_span,
        sugars_profile=sugars_profile,
        Et_final_exp=Et_final_exp,
        penalty=penalty
    )

    return breakdown["objective_total"]


def objective_function_multi(
    theta,
    free_names,
    fixed_params,
    datasets,
    penalty=1e12
):
    """
    Función objetivo promedio para múltiples fermentaciones.
    """

    n_fermentations = len(datasets)

    if n_fermentations == 0:
        raise ValueError("datasets no puede estar vacío")

    total_cost = 0.0

    for data in datasets:
        total_cost += objective_function(
            theta=theta,
            free_names=free_names,
            fixed_params=fixed_params,
            x0=data["x0"],
            t_rel=data["t_rel"],
            temp=data["temp"],
            Nadd=data["Nadd"],
            t_span=data["t_span"],
            sugars_profile=data["sugars_profile"],
            Et_final_exp=data["Et_final_exp"],
            penalty=penalty
        )

    return float(total_cost / n_fermentations)


def objective_function_swarm_multi(
    swarm,
    free_names,
    fixed_params,
    datasets,
    penalty=1e12
):
    """
    Evalúa una población de partículas para pymoo.
    """

    swarm = np.atleast_2d(swarm)
    costs = np.empty(swarm.shape[0], dtype=float)

    for i, theta in enumerate(swarm):
        costs[i] = objective_function_multi(
            theta=theta,
            free_names=free_names,
            fixed_params=fixed_params,
            datasets=datasets,
            penalty=penalty
        )

    return costs


# =============================================================================
# 8. Problema para pymoo
# =============================================================================

class FermentationPymooProblem(Problem):
    """
    Problema de optimización para calibración de parámetros.
    """

    def __init__(self, free_names, fixed_params, datasets, xl, xu):
        super().__init__(
            n_var=len(free_names),
            n_obj=1,
            xl=xl,
            xu=xu
        )

        self.free_names = free_names
        self.fixed_params = fixed_params
        self.datasets = datasets

    def _evaluate(self, X, out, *args, **kwargs):
        F = objective_function_swarm_multi(
            swarm=X,
            free_names=self.free_names,
            fixed_params=self.fixed_params,
            datasets=self.datasets
        )

        out["F"] = F.reshape(-1, 1)


# =============================================================================
# 9. Criterio de término por brecha relativa
# =============================================================================

class RelativeGapTermination(Termination):
    """
    Detiene la optimización cuando:

        (f_avg - f_min) / |f_min| < threshold

    Esto indica que la población ya está muy concentrada en torno al mejor valor.
    """

    def __init__(self, threshold=0.05, eps=1e-12):
        super().__init__()

        self.threshold = threshold
        self.eps = eps

        self.last_f_avg = None
        self.last_f_min = None
        self.last_ratio = None

        self.stop_iteration = None
        self.stop_reason = None

    def _update(self, algorithm):
        F = algorithm.pop.get("F")

        if F is None or len(F) == 0:
            return 0.0

        F = np.asarray(F, dtype=float).reshape(-1)

        f_avg = float(np.mean(F))
        f_min = float(np.min(F))

        denom = max(abs(f_min), self.eps)
        ratio = (f_avg - f_min) / denom

        self.last_f_avg = f_avg
        self.last_f_min = f_min
        self.last_ratio = ratio

        if ratio < self.threshold:
            self.stop_iteration = algorithm.n_gen
            self.stop_reason = "relative_gap"
            return 1.0

        return 0.0


# =============================================================================
# 10. Ejecución principal de PSO
# =============================================================================

def run_pymoo_estimation(
    model_structure,
    datasets,
    pso_config=None
):
    """
    Ejecuta PSO con pymoo para calibrar los parámetros libres del modelo.

    Parameters
    ----------
    model_structure : dict
        Estructura con parámetros fijos y libres.

    datasets : list[dict]
        Lista de fermentaciones. Cada dataset debe contener:
            x0
            t_rel
            temp
            Nadd
            t_span
            sugars_profile
            Et_final_exp

    pso_config : dict, optional
        Configuración personalizada de PSO.

    Returns
    -------
    result : dict
        Resultado resumido de la optimización.

    best_params : dict
        Diccionario completo con parámetros fijos y calibrados.
    """

    fixed_params, free_names = prepare_model_structure(model_structure)

    bounds = build_bounds_for_free_params(
        free_names=free_names,
        bounds_dict=BOUNDS_DICT
    )

    config = get_pso_config(pso_config)

    epoch = config["epoch"]
    pop_size = config["pop_size"]
    w = config["w"]
    c1 = config["c1"]
    c2 = config["c2"]
    seed = config["seed"]
    verbose = config["verbose"]
    save_history = config.get("save_history", False)
    relative_gap_threshold = config.get("relative_gap_threshold", 0.05)

    lb = np.array([b[0] for b in bounds], dtype=float)
    ub = np.array([b[1] for b in bounds], dtype=float)

    problem = FermentationPymooProblem(
        free_names=free_names,
        fixed_params=fixed_params,
        datasets=datasets,
        xl=lb,
        xu=ub
    )

    algorithm = PymooPSO(
        pop_size=pop_size,
        w=w,
        c1=c1,
        c2=c2,
        adaptive=False
    )

    relative_gap_termination = RelativeGapTermination(
        threshold=relative_gap_threshold
    )

    max_gen_termination = get_termination("n_gen", epoch)

    termination = TerminationCollection(
        relative_gap_termination,
        max_gen_termination
    )

    pymoo_result = minimize(
        problem,
        algorithm,
        termination=termination,
        seed=seed,
        verbose=verbose,
        save_history=save_history,
    )

    best_theta = np.asarray(pymoo_result.X, dtype=float)
    best_cost = float(np.atleast_1d(pymoo_result.F)[0])

    best_params = build_full_params(
        theta=best_theta,
        free_names=free_names,
        fixed_params=fixed_params
    )

    history_f_min = []
    history_f_avg = []

    if save_history and getattr(pymoo_result, "history", None):
        for algo in pymoo_result.history:
            F = algo.pop.get("F")
            F = np.asarray(F, dtype=float).reshape(-1)

            history_f_min.append(float(np.min(F)))
            history_f_avg.append(float(np.mean(F)))

    if relative_gap_termination.stop_iteration is None:
        stop_iteration = pymoo_result.algorithm.n_gen
        stop_reason = "max_epoch"
    else:
        stop_iteration = relative_gap_termination.stop_iteration
        stop_reason = relative_gap_termination.stop_reason

    final_ratio = relative_gap_termination.last_ratio
    final_f_avg = relative_gap_termination.last_f_avg
    final_f_min = relative_gap_termination.last_f_min

    if final_ratio is None:
        F = pymoo_result.pop.get("F")
        F = np.asarray(F, dtype=float).reshape(-1)

        final_f_avg = float(np.mean(F))
        final_f_min = float(np.min(F))

        denom = max(abs(final_f_min), 1e-12)
        final_ratio = (final_f_avg - final_f_min) / denom

    print(f"Motivo de término: {stop_reason}")
    print(f"Iteración de término: {stop_iteration}")
    print(f"Valor final de (f_avg - f_min)/f_min: {final_ratio:.6f}")
    print("")
    print("Mejores parámetros encontrados:")
    for name in PARAM_ORDER:
        print(f"{name}: {best_params[name]:.8g}")

    result = {
        "x": best_theta,
        "fun": best_cost,
        "free_names": free_names,
        "fixed_params": fixed_params,
        "best_params": best_params,
        "history": {
            "f_min": history_f_min,
            "f_avg": history_f_avg,
        },
        "method": "pso_pymoo",
        "raw_result": pymoo_result,
        "termination_info": {
            "stop_reason": stop_reason,
            "stop_iteration": stop_iteration,
            "final_ratio": final_ratio,
            "final_f_avg": final_f_avg,
            "final_f_min": final_f_min,
            "threshold": relative_gap_threshold,
            "max_epoch": epoch,
        }
    }

    return result, best_params


# =============================================================================
# 11. Gráfico de convergencia
# =============================================================================

def plot_pymoo_history(result, title="Convergencia del PSO con pymoo"):
    """
    Grafica f_min y f_avg a lo largo de las iteraciones.
    """

    history = result.get("history", None)

    if history is None:
        print("No hay historial disponible.")
        return

    f_min = history.get("f_min", [])
    f_avg = history.get("f_avg", [])

    if len(f_min) == 0:
        print("El historial está vacío. Revisa que save_history=True.")
        return

    generations = np.arange(1, len(f_min) + 1)

    plt.figure(figsize=(8, 4))
    plt.plot(generations, f_min, label="f_min")

    if len(f_avg) == len(f_min):
        plt.plot(generations, f_avg, label="f_avg")

    plt.xlabel("Iteración")
    plt.ylabel("Valor objetivo")
    plt.title(title)
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()


# =============================================================================
# 12. Utilidad rápida para revisar estructura del modelo
# =============================================================================

def print_model_structure(model_structure=MODEL_2264):
    """
    Imprime la estructura de parámetros libres y fijos.
    """

    fixed_params, free_names = prepare_model_structure(model_structure)

    print("Parámetros libres:")
    for name in free_names:
        print(f"  - {name}: bounds = {BOUNDS_DICT[name]}")

    print("")
    print("Parámetros fijos:")
    for name, value in fixed_params.items():
        print(f"  - {name}: {value:.8g}")

    print("")
    print("Orden completo usado por el modelo:")
    for name in PARAM_ORDER:
        source = "libre" if name in free_names else "fijo"
        print(f"  - {name}: {source}")